...
## Helpers
    def L_i(self, cur_point, next_point):
        result = np.linalg.norm(cur_point - next_point)
    ...
    def update_trajectory(self, T, K_list, lr=0.00001):
        ...
        while True:
            ...
            for i, vertex in enumerate(T):
                ...
                potential_sum = np.append(potential_sum, self.potential_value_p(vertex, K_list))
            print("Potential sum array: ", potential_sum)
            print("potential sum: ", np.sum(potential_sum))
            if np.sum(potential_sum) < 0.01:
                break
        return new_vertices