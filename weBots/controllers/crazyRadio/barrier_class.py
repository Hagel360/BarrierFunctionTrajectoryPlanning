...
class Barrier:
    ...
    def update_trajectory(self, T, K_list, lr=0.00000001):
        ...
        for i, vertex in enumerate(T):
            ...
            for K in K_list:
                ...
                for point in K:
                    ...
                    # if distance is zero, add a small value to avoid division by zero
                    # and add noise to vertex and point
                    if distance == 0:
                        vertex += np.random.normal(0, 0.001, vertex.shape)
                        point += np.random.normal(0, 0.001, point.shape)
                        distance = np.linalg.norm(vertex - point)
